local DefaultMatcher = require('autotest.poco.sdk.DefaultMatcher')
...
function Selector:selectImpl(cond, multiple, root, maxDepth, onlyVisibleNode, includeRoot)
    ...
    if op == '>' or op == '/' then
        ...
        for index, arg in ipairs(args) do
            ...
            for _, parent in ipairs(parents) do
                ...
                local _res = self:selectImpl(arg, true, parent, _maxDepth, onlyVisibleNode, false)
    ...
    elseif op == '-' then
        -- 兄弟节点选择
        local query1, query2 = table.unpack(args)
        local result1 = self:selectImpl(query1, multiple, root, maxDepth, onlyVisibleNode, includeRoot)
        for _, n in ipairs(result1) do
            local sibling_result = self:selectImpl(query2, multiple, n:getParent(), 1, onlyVisibleNode, includeRoot)
    ...
    elseif op == 'index' then
        local cond, i = table.unpack(args)
    ...
    elseif op == '^' then
        ...
        local query1, _ = table.unpack(args)
