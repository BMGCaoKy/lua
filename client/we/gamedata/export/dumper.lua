...
function dump_value(value, depth)
	local t = type(value)
	--TODO 根据数组的结构判断是否是导出一个数组
	--蓝图数组结构:value是该数组，但是每一个成员的数据在rawval中。
	--如果value碰巧结构与数组一致，会导出错误
	if t == "table" and value[1] and type(value[1]) == "table" and value[1]["rawval"] then
		for  k, v in ipairs(value) do
			value[k] = value[k]["rawval"]
		end
	end
	if t ~= "table" then
...
function do_dump_action(action, depth)
	...
	elseif type == "GetGlobalVar" or type == "SetGlobalVar"
		...
		or type == "GetObjectVar" or type == "SetObjectVar" or type == "GetContextParam" then
		return dump_sugar(action, depth)
	elseif type == "ArrayInsert" or type == "ArraySet"
		or type == "ArrayAppend" or type == "ArrayRemove"
		or type == "ArrayGet" or type == "ArraySize"
		or type == "ArrayFind"or type == "IsValueInArray"then
		return dump_array_action(action, depth)
...
function dump_sugar(action, depth)
	...
	elseif type(key) == "string" then
		...
		local sign = Lib.splitIncludeEmptyString(params.key, "/")
		--避免导出/error
		if sign[#sign] == "error" then
			table.remove(sign)
			local value_key = table.concat(sign,"/").."_error"
			ret = strfmt("%s%s", prefix, value_key)
		end
	else
...
function dump_array_action(action, depth)
	local object = ""
	local map = {}
	local order = {}
	local params = action.params
	for _, k in pairs(get_sorted_keys(params)) do
		if k ~= "type" then
			order[#order + 1] = k
		end
	end
	...
	--变量的前缀 @a $obj.a  createobject().a
	local var_prefix =""
	if params.type =="global" then
		var_prefix = "@"
	elseif params.type =="ui" and action.funcs.rootInstance then
		var_prefix = strfmt("GetUIVar(key = \"%s\", instance = %s)", params.array,dump_action(action.funcs.rootInstance, depth))
	elseif action.funcs.entity then
		var_prefix = dump_action(action.funcs.entity,depth).."."
	end

	local list = {}
	local ok_map = {rootInstance = true}
	for _, k in ipairs(order) do
		if ok_map[k] then
			goto continue
		end
		...
		if k =="array" then
			if params[k] ~= "" and var_prefix ~= "" then
				if params.type =="ui" then
					list[#list + 1] = "array = "..var_prefix
				else
					list[#list + 1] = "array = "..var_prefix..params[k]
				end
			end
	...
	local prefix = depth and "\n"..string.rep("\t", depth + 1) or ""
	return strfmt("%s(%s%s)", action.type,prefix..table.concat(list, depth and ","..prefix or ", "), prefix:sub(1, -2))
end
