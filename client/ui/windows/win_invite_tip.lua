...
function M:onOpen(packet)
    ...
    self.cfg = setting:fetch("ui_config", not string.find( packet.fullName, base_plugin .. "/") and (base_plugin .. "/" .. packet.fullName) or packet.fullName)
    self:updateBgImage()
...
function M:onClose(eventKey)
    ...
    if eventKey and self.regId then
        Me:doCallBack("ShowInviteTip", eventKey, self.regId)
    end
    self.regId = nil
end
...
local function _updateBtnText(btn, btnInfo)
