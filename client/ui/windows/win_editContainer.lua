...
local function startTouchListener(self, objID)
    ...
    local touchTick = 0
    tlco.beginTouchEventListener = Lib.lightSubscribeEvent("error!!!!! script_client win_editContainer Lib event : EVENT_SCENE_TOUCH_BEGIN",Event.EVENT_SCENE_TOUCH_BEGIN, function(x, y)
        touchTick = curWorld:getTickCount()
    end)
    tlco.endTouchEventListener = Lib.lightSubscribeEvent("error!!!!! script_client win_editContainer Lib event : EVENT_SCENE_TOUCH_END",Event.EVENT_SCENE_TOUCH_END, function(x, y)
...
function M:initEvent()
    Lib.lightSubscribeEvent("error!!!!! script_client win_editContainer Lib event : EVENT_UI_EDIT_UPDATE_EDIT_CONTAINER",Event.EVENT_UI_EDIT_UPDATE_EDIT_CONTAINER, function(objID, show)
        self.allCell[objID] = {}
        ...
        DrawRender.instance:setDrawSelectedColliderAABBEnabled(not not show)
...
calcBoundingBoxTouchBlock = function(worldPos, entity, sideNormal)
    ...
    local boundBoxSize = {x = 0, y = 0, z = 0}
    if boundingBox then
        boundBoxSize = Lib.v3cut(boundingBox[3], boundingBox[2])
    ...
    for i = minPosX, maxPosX do
        for j = minPosY, maxPosY do
            for k = minPosZ, maxPosZ do
                ...
                if block.fullName ~= airFullName then
...
local function getEntityPos(mouseHit, objID)
    ...
    if mouseHit.type=="ENTITY" then
        ...
        if entity.objID ~= objID then
            ...
            local boundingBox = entity:getBoundingBox()
            if entity:cfg().supportingForce and boundingBox then
                pos.y = boundingBox[3].y
            end
        end
...
local function displayMoveEvent(self, objID, cell, moveWidget, editUI)
    ...
    local params = {}
    local showFunc
    self:lightSubscribe("error!!!!! script_client win_editContainer displayMoveEvent-cell-objID"..objID.." event : EventWindowTouchDown",cell, UIEvent.EventWindowTouchDown, function()
        ...
        self:lightSubscribe("error!!!!! script_client win_editContainer self.touchEndBg event : EventWindowTouchUp",self.touchEndBg, UIEvent.EventWindowTouchUp, function()
...
function M:createAroundWidget(objID, aroundWidget, editUI)
    ...
    for index, weight in ipairs(aroundWidget) do
        ...
        if weight.action then
            self:lightSubscribe("error!!!!! script_client win_editContainer createAroundWidget-cell-action-objID"..objID.." event : EventWindowClick",cell, UIEvent.EventWindowClick, function()
                sendEditObject(objID, weight.action, {})
            end)
        elseif weight.tipContent then
            self:lightSubscribe("error!!!!! script_client win_editContainer createAroundWidget-cell-tipContent-objID"..objID.." event : EventWindowClick",cell, UIEvent.EventWindowClick, function()
