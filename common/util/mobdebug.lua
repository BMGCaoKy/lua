...
local loadstring = rawget(_G, "loadstring") or load -- "load" replaced "loadstring" in Lua 5.2
local pairs = pairs
...
local socket = require "libraries.luasocket.socket"
local coro_debugger
local coro_debugee
...
local function controller(controller_host, controller_port, scratchpad)
  ...
  if server then
    ...
    while true do
      ...
      local status, err = cororesume(coro_debugee, unpack(rawget(_G, "arg") or {}))
