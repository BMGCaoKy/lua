...
function EmojiConfig:init()
    local csvData = Lib.read_csv_file(Root.Instance():getGamePath() .. "lua/plugins/platform_chat/csv/emoji.csv", 2)
    if not csvData then
        print("cant find game config/voiceShop.csv,try use plugins defualt!")
        csvData = Lib.read_csv_file(Root.Instance():getRootPath() .. "lua/plugins/platform_chat/csv/emoji.csv", 2) or {}
    end
